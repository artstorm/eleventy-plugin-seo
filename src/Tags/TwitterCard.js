...
class TwitterCard extends BaseTag {
  async liquidRender(scope, hash) {
    ...
    const siteTwitter = !scope.contexts[0].siteTwitter && this.config.twitter ? this.config.twitter : scope.contexts[0].siteTwitter;

    // Define and update a new template context
    const templateContext = { ...scope.contexts[0], siteTwitter, image: imageWithBaseUrl ? baseImage : image };
  ...
  nunjucksRender(self, context) {
    ...
    // Get twitter username for site from config.
    const siteTwitter = !context.ctx.siteTwitter && self.config.twitter ? self.config.twitter : context.ctx.siteTwitter;

    // Define and update a new template context
    const templateContext = { ...context.ctx, siteTwitter, image: imageWithBaseUrl ? baseImage : image };
